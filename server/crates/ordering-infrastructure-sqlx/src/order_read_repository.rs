...
impl OrderReadRepository for SqlxOrderReadRepository {
    async fn get_by_id(
        ...
        Ok(Some(OrderReadModel {
            ...
            status: to_status_string(row.get::<DbOrderStatus, _>("status")),
            ...
            items: item_rows
                ...
                .map(|row| OrderItemReadModel {
                    ...
                    catalog_item_id: row.get::<Uuid, _>("catalog_item_id").to_string(),
                    name: row.get("name"),
...
fn to_status_string(value: DbOrderStatus) -> String {
    match value {
        DbOrderStatus::Placed => "placed",
        DbOrderStatus::CancelledByCustomer => "cancelled_by_customer",
    }
    .to_string()
}
