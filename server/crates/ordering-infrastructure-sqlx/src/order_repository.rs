...
use ordering_food_ordering_application::{ApplicationError, OrderRepository, TransactionContext};
use ordering_food_ordering_domain::{
    CatalogItemId, CustomerId, Order, OrderId, OrderItem, OrderStatus, StoreId,
};
    ...
    async fn load_items(
        ...
        rows.into_iter()
            .map(|row| {
                OrderItem::rehydrate(
                    row.get("line_number"),
                    CatalogItemId::new(row.get::<Uuid, _>("catalog_item_id").to_string()),
