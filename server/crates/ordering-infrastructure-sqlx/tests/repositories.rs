use ordering_food_ordering_application::{
    OrderReadRepository, OrderRepository, TransactionManager,
};
...
fn make_order(order_id: Uuid, customer_id: Uuid, store_id: Uuid) -> Order {
    Order::place(
        ...
        vec![
            PlaceOrderItemInput {
                catalog_item_id: CatalogItemId::new(Uuid::now_v7().to_string()),
...
async fn sqlx_order_repositories_persist_and_load_snapshot(pool: PgPool) {
    ...
    assert_eq!(read_model.status, "placed");
...
async fn postgres_enum_round_trips_with_domain_status(pool: PgPool) {
    ...
    assert_eq!(OrderStatus::parse(status).unwrap(), OrderStatus::Placed);
}
