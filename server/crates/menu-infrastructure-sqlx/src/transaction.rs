...
use ordering_food_menu_application::{ApplicationError, TransactionContext, TransactionManager};
...
impl SqlxTransactionContext {
    ...
    async fn commit(self: Box<Self>) -> Result<(), ApplicationError> {
        ...
        transaction.commit().await.map_err(|error| {
            ApplicationError::unexpected_with_source("failed to commit menu transaction", error)
        })
    }
    ...
    async fn rollback(self: Box<Self>) -> Result<(), ApplicationError> {
        ...
        transaction.rollback().await.map_err(|error| {
            ApplicationError::unexpected_with_source("failed to rollback menu transaction", error)
        })
    }
}
...
impl TransactionManager for SqlxTransactionManager {
    async fn begin(&self) -> Result<Box<dyn TransactionContext>, ApplicationError> {
        let transaction = self.pool.begin().await.map_err(|error| {
            ApplicationError::unexpected_with_source("failed to begin menu transaction", error)
        })?;
