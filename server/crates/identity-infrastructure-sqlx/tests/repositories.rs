...
use ordering_food_identity_application::{
    ApplicationError, IdentityUnitOfWorkFactory, UserReadRepository,
};
...
use ordering_food_identity_infrastructure_sqlx::{
    SqlxIdentityUnitOfWorkFactory, SqlxUserReadRepository,
};
...
async fn insert_user(factory: &SqlxIdentityUnitOfWorkFactory, user: &User) {
...
async fn sqlx_user_repository_returns_none_for_missing_identity(pool: PgPool) {
    let factory = SqlxIdentityUnitOfWorkFactory::new(pool);
    let mut unit_of_work = factory.begin().await.unwrap();
...
async fn sqlx_user_repository_updates_profile_status_and_identities(pool: PgPool) {
    ...
    let mut unit_of_work = factory.begin().await.unwrap();
    unit_of_work.update_user(&user).await.unwrap();
    unit_of_work.commit().await.unwrap();
...
async fn sqlx_user_repository_update_returns_not_found_for_missing_user(pool: PgPool) {
    ...
    let error = unit_of_work.update_user(&missing_user).await.unwrap_err();
    unit_of_work.rollback().await.unwrap();
...
async fn sqlx_user_repository_rejects_invalid_uuid_string(pool: PgPool) {
    ...
    let error = unit_of_work
        .find_user_by_id(&UserId::new("not-a-uuid"))
        .await
        .unwrap_err();
    unit_of_work.rollback().await.unwrap();
...
async fn sqlx_credential_repository_upsert_updates_hash_and_timestamp(pool: PgPool) {
    ...
    let mut unit_of_work = factory.begin().await.unwrap();
    unit_of_work
        .upsert_credential(user.id(), "hashed:first", fixed_timestamp(1_700_000_700))
        .await
        .unwrap();
    unit_of_work.commit().await.unwrap();
    ...
    let mut unit_of_work = factory.begin().await.unwrap();
    unit_of_work
        .upsert_credential(user.id(), "hashed:second", fixed_timestamp(1_700_000_900))
        .await
        .unwrap();
    unit_of_work.commit().await.unwrap();
...
async fn sqlx_credential_repository_rejects_invalid_uuid_string(pool: PgPool) {
    ...
    let mut unit_of_work = factory.begin().await.unwrap();
    let error = unit_of_work
        .find_credential_by_user_id(&UserId::new("not-a-uuid"))
        .await
        .unwrap_err();
    unit_of_work.rollback().await.unwrap();
