use ordering_food_identity_application::ApplicationError;
...
pub(crate) async fn find_user_by_id(
    ...
    load_user(transaction, user_id).await
}

pub(crate) async fn find_user_by_identity(
    transaction: &mut Transaction<'static, Postgres>,
    ...
    match user_id {
        Some(user_id) => load_user(transaction, &user_id).await,
        None => Ok(None),
    }
}
...
pub(crate) async fn insert_user(
    ...
    .await
    .map_err(|error| {
        ApplicationError::unexpected_with_source("failed to insert identity user profile", error)
    })?;
    ...
    for identity in user.identities() {
        ...
        .await
        .map_err(|error| {
            map_identity_write_error("failed to insert identity user identity", error)
        })?;
    }
...
pub(crate) async fn update_user(
    ...
    .await
    .map_err(|error| {
        ApplicationError::unexpected_with_source("failed to update identity user profile", error)
    })?;
...
async fn load_user(
    ...
    .await
    .map_err(|error| {
        ApplicationError::unexpected_with_source("failed to load identity user aggregate", error)
    })?;
    ...
    .await
    .map_err(|error| {
        ApplicationError::unexpected_with_source("failed to load identity user identities", error)
    })?
    .into_iter()
    .map(|identity| {
        Ok(UserIdentity::new(
...
fn map_identity_write_error(message: &'static str, error: sqlx::Error) -> ApplicationError {
    if is_identity_uniqueness_violation(&error) {
...
fn is_identity_uniqueness_violation(error: &sqlx::Error) -> bool {
    error.as_database_error().is_some_and(|database_error| {
        matches_identity_uniqueness_violation(
...
#[cfg(test)]
mod tests {
    use super::matches_identity_uniqueness_violation;

    #[test]
    fn matches_identity_unique_violation_for_target_constraint() {
        assert!(matches_identity_uniqueness_violation(
            Some("23505"),
