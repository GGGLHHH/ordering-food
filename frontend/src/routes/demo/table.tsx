...
function TableDemo() {
  ...
  const activeColumnFilterId = table.getState().columnFilters[0]?.id
  const activeSortingId = table.getState().sorting[0]?.id

  //apply the fuzzy sort if the fullName column is being filtered
  React.useEffect(() => {
    if (activeColumnFilterId === 'fullName' && activeSortingId !== 'fullName') {
      table.setSorting([{ id: 'fullName', desc: false }])
    }
  }, [activeColumnFilterId, activeSortingId, table])
